...
class AnalogDiscovery2(IODevice):
    ...
    def start_ao(self, channel=Echannel.ch_1):
        dwf.FDwfAnalogOutConfigure(self.hdwf, c_int(channel.value), c_int(1))
        self.check_ao = True
    ...
    def create_square_wave(self, outputWave, outVoltage=1.0, channel=Echannel.ch_1):
        channel = c_int(channel)
        outputWave = float(outputWave)
        dwf.FDwfAnalogOutNodeEnableSet(self.hdwf, channel.value, dwfc.AnalogOutNodeCarrier, c_bool(True))
        dwf.FDwfAnalogOutNodeFunctionSet(self.hdwf, channel.value, dwfc.AnalogOutNodeCarrier, dwfc.funcSquare)
        dwf.FDwfAnalogOutNodeSymmetrySet(self.hdwf, channel.value, dwfc.AnalogOutNodeCarrier, c_double(50))
        dwf.FDwfAnalogOutNodeFrequencySet(self.hdwf, channel.value, dwfc.AnalogOutNodeCarrier, c_double(outputWave))
...
if __name__ == "__main__":
    ...
    wave = [0.95 if i < 200 else 0 for i in range(400)]
    zero = [0 for i in range(400)]
    device.create_custom_wave(customWave=wave, period=220, outVoltage=5, channel=Echannel.ch_1)
    device.create_custom_wave(customWave=wave, period=220, outVoltage=5, channel=Echannel.ch_2)
    #device.create_custom_wave(channel=Echannel.ch_2)
    ### 単振動出力 ###
